...
{
    public class UpdateProductPageViewModel : BasePageViewModel<UpdateProductPageViewModel, IProductService>, IAutoInitialize, IViewModelExtensions
    {
        #region Services
        ...
        private Operation _operation = Operation.Modified;
        ...
        //private DelegateCommand _openUnitsPickerCommand;
        //public DelegateCommand OpenUnitsPickerCommand => _openUnitsPickerCommand ?? (_openUnitsPickerCommand = new DelegateCommand(() => OpenUnitPickerCommandExecute()));
        ...
        public DelegateCommand RemoveProductCommand =>
          _removeProductCommand ?? (_removeProductCommand = new DelegateCommand(async () => await RemoveProductCommandExecute()));


        private DelegateCommand _removeUnitsCommand;
        public DelegateCommand RemoveUnitsCommand =>
          _removeUnitsCommand ?? (_removeUnitsCommand = new DelegateCommand(async () => await RemoveUnitsCommandExecute()));

        public UpdateProductPageViewModel(INavigationService navigationService, IPageDialogService pageDialogService, IAppSettings settings, IEventAggregator eventAggregator, IProductService productService, IUnitService unitService, ICategoryService categoryService, IMeasurementTypeService measurementTypeService) : base(navigationService, pageDialogService, settings, eventAggregator)
        {
            ...
            ProductCategory = new PickerItem() { Id = 0, Description = "Select Product", Text = "Select Product" };
        ...
        private async void OpenUnitPickerCommandExecute()
        {
            var CategoryItems = await _unitService.GetLocalUnitsModel();
            List<PickerItem> pickerItems = new List<PickerItem>();
            pickerItems = CategoryItems.Select(a => new PickerItem()
            {
                Id = a.LocalId,
                Description = a.UnitTitle,
                Text = a.UnitTitle,
                PickerItemType = PickerTypesEnums.Units
            }).ToList();
            await ShowPopup(pickerItems);
        }
        public UpdateProductPageViewModel() : base()
        {
            ValidateName = new ValidatableObject<string>();
            ValidateProductCategory = new ValidatableObject<PickerItem>();
            ValidateProductMeasurementType = new ValidatableObject<PickerItem>();
            ...
            //ProductCategory = new PickerItem() { Id = 0, Description = "Select category", Text = "Select category" };
            //ValidateProductCategory.Value = ProductCategory;
            AddValidations();
        }
        public async override Task UpdateModel(UpdateProductPageViewModel model)
        {
            ...
            var ViewModel = await DataService.UpdateProductReturnViewModel<UpdateProductPageViewModel>(model);
            LocalId = ViewModel.LocalId;
            await NavigationService.GoBackAsync();
        }

        public override UpdateProductPageViewModel GetData()
        {
            if (PageRequest.LocalId > 0 && PageRequest.LocalId > -1)
            {
                var GetLocalProducById = DataService.GetLocalProductById<UpdateProductPageViewModel>(PageRequest.Id);
                _validateName.Value = GetLocalProducById.Name;
                var productCategroy = _categoryService.GetLocalCategoryById<CategoryThinViewModel>(GetLocalProducById.CategorysId);

                var MeasurementType = _measurementTypeService.GetLocalMeasurementTypesModelById(GetLocalProducById.MeasurementTypeId).GetAwaiter().GetResult();

                ProductMeasurementTypePicker = new PickerItem()
                {
                    Description = MeasurementType.Descriptions,
                    Text = MeasurementType.MeasurementTypeName,
                    PickerItemType = PickerTypesEnums.MeasurementType,
                    Id = MeasurementType.LocalId
                };
                //var productUnit = _unitService.GetLocalUnitsById<UnitsThinViewModel>(GetLocalProducById.UnitsId);
                ProductCategory = new PickerItem()
                {
                    Description = productCategroy.Description,
                    Text = productCategroy.Name,
                    PickerItemType = PickerTypesEnums.Categories,
                    Id = productCategroy.LocalId
                };


                // ValidateProductCategory.Value = ProductCategory;
                //ProductUnit =   new PickerItem()
                // {
                //     Description = productUnit.UnitTitle,
                //     Text = productUnit.UnitTitle,
                //     PickerItemType = PickerTypesEnums.Units,
                //     Id = productUnit.LocalId
                // };
                //ValidateProductUnits.Value = ProductUnit;
                ValidateProductCategory.Value = ProductCategory;
                ValidateProductMeasurementType.Value = ProductMeasurementTypePicker;
                //ValidateProductUnits.Value = ProductUnit;
                ValidateName.Value = GetLocalProducById.Name;
                if (GetLocalProducById == null)
                {
                    IsBusy = false;
                    IsEmpty = true;
                    EmptyStateTitle = AlertMessages.EmptyState.DefaultTitle;
                    EmptyStateSubtitle = AlertMessages.EmptyState.DefaultSubtitle;
                }
                return GetLocalProducById;
            }
            else
            {
                return DataService.GetLocalProductById<UpdateProductPageViewModel>(PageRequest.Id);
            }
        }
        ...
        public async override Task RefreshData()
        {
            ...
            {
                Debug.WriteLine("ERROR loading case: ex: " + ex);
            }
        ...
        private async Task SaveProductCommandExecute()
        {
            ...
            if (ValidateName.IsValid && ValidateProductCategory.IsValid)
            {
                IsBusy = true;

                //UnitsId = ValidateProductUnits.Value.Id;
        ...
        private async Task RemoveProductCommandExecute()
        {
            try
            {
                ...
                var ConfimAsync = await UserDialogsService.ConfirmAsync("All Stock related entry of this shade  should bee deleted. Are you sure want remove this shade ?", null, "Yes", "No");
                if (ConfimAsync)
                {
                    IsDelete = true;
                    IsActive = true;
                    Operation = Operation.Deleted;
                    await SaveProductCommandExecute();
                }
        ...
        private async Task RemoveUnitsCommandExecute()
        {
